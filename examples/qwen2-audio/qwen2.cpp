...
struct omni_streaming {
    ...
    struct llama_sampling_context* ctx_sampling_;
    ...
    omni_streaming(omni_context* ctx, const omni_params& params)
        ...
        dec_cnt_ = 0;
        n_past_ = 0;
        ctx_sampling_ = llama_sampling_init(params_.gpt.sparams);
    };

    int32_t sample() {
        llama_token id = llama_sampling_sample(ctx_sampling_, ctx_omni_->ctx_llama, NULL);
        llama_sampling_accept(ctx_sampling_, ctx_omni_->ctx_llama, id, true);
        static std::string ret_str;
        if (llama_token_is_eog(llama_get_model(ctx_omni_->ctx_llama), id)) {
            ret_str = "</s>";
        } else {
            ret_str = llama_token_to_piece(ctx_omni_->ctx_llama, id);
        }
